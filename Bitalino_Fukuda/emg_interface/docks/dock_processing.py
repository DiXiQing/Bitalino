...
class ProcessingDock(BaseDock):
    ...
    def __init__(self, num_channels=4):
        ...
        self.max_value_groupbox = CalibrationGroupBox(  #maxmin 9秒間
            num_channels, duration=9, parent=self
        )
    ...
    def gui_save(self, settings):
        ...
        for i, spinbox in enumerate(self.zero_offset_groupbox.spinboxes):
            name = f"zero_offset_spinbox{i}"
            value = spinbox.value()
            settings.setValue(f"{self.save_heading}/{name}", value)

        for i, spinbox in enumerate(self.max_value_groupbox.spinboxes):
            name = f"maxmin_offset_spinbox{i}"
            value = spinbox.value()
            settings.setValue(f"{self.save_heading}/{name}", value)
    ...
    def gui_restore(self, settings):
        ...
        for i, spinbox in enumerate(self.zero_offset_groupbox.spinboxes):
            name = f"zero_offset_spinbox{i}"
            value = settings.value(f"{self.save_heading}/{name}" )  
            if value:         
